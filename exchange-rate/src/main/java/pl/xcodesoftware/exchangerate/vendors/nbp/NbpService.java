...
public class NbpService {
    ...
    public ExchangeRate getExchangeRate(CurrencyShortcut currencyShortcut) {
        try {
            ...
            var nbpExchangeRate = restTemplate
                    .getForObject(String.format("%1$s%2$s", URI_NBP, currencyShortcut.getCurrency()), NbpExchangeRate.class);

            assert nbpExchangeRate != null;
            return new ExchangeRate(nbpExchangeRate.getRates().get(0).getMid());

        } catch (HttpClientErrorException e) {

            log.error(e.getMessage());
            switch(e.getStatusText()) {
                case NOT_FOUND -> throw new NbpDataException$NotFound("No data.");
